...
interface CourseState {
    ...
    batchUpdateFilters: (
        filters: Partial<
            Pick<CourseState, "countryFilter" | "selectedUniversity" | "studyLevel" | "subjectAreaFilter" | "search">
        >,
    ) => void
}
    ...
    batchUpdateFilters: (filters) => {
        set((state) => ({
            ...
            countryFilter: filters.countryFilter
                ? Array.isArray(filters.countryFilter)
                    ? filters.countryFilter
                    : []
                : state.countryFilter,
            subjectAreaFilter: filters.subjectAreaFilter
                ? Array.isArray(filters.subjectAreaFilter)
                    ? filters.subjectAreaFilter
                    : []
                : state.subjectAreaFilter,
        }))
    },
    fetchCourses: async () => {
        console.log("[v0] fetchCourses called with current state:", {
            search: get().search,
            selectedUniversity: get().selectedUniversity,
            countryFilter: get().countryFilter,
            studyLevel: get().studyLevel,
            subjectAreaFilter: get().subjectAreaFilter,
        ...
        try {
            ...
            if (selectedUniversity) queryParams.append("selectedUniversity", selectedUniversity)
            ...
            console.log("[v0] Making API call:", `/api/getCourses?${queryParams.toString()}`)
