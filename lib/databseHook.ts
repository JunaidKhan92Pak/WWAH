    ...
    import { CountryDocumentData } from "./embedding-operations";
    ...
    async createDocument(
        ...
        if (collectionName === "userdbs") {
            ...
            if (successChances.length > 0) {
                enhancedDocument.successChanceData = successChances[0]; // Use first success chance for embedded data
                console.log(
    ...
    return await hooks.createDocument("countries", countryData as Record<string, unknown>);
    }
    ...
    if (ObjectId.isValid(countryId)) {
        ...
        // If it's not a valid ObjectId, treat it as country name
        filter = { country_name: countryId };
    }
    ...
    const hooks = new DatabaseHooks(client);
    return await hooks.deleteDocument("universities", { _id: universityId });
    }
    ...
    const hooks = new DatabaseHooks(client);
    return await hooks.deleteDocument("expenses", { _id: expenseId });
    }
