...
export const calculateEnglishSuccess = (userLang: string, userLangScore: number, scholarShipLang: string) => {
    if (!userLang || !scholarShipLang) return 0;

    const userTest = userLang || "IELTS";
    const userScore = userLangScore || 0;
    const englishRequirements = extractLangScore(scholarShipLang);

    let requiredScore = 0;
    if (userTest === "IELTS") {
        requiredScore = englishRequirements.ielts;
    } else if (userTest === "PTE") {
        requiredScore = englishRequirements.pte;
    } else if (userTest === "TOEFL") {
        requiredScore = englishRequirements.toefl;
    }

    // Calculate success percentage
    if (requiredScore === 0) return 10; // No requirement available
    if (userScore >= requiredScore) return 100; // Meets or exceeds requirement
    if (userScore >= requiredScore * 0.95) return 90; // Very close
    if (userScore >= requiredScore * 0.9) return 80; // Close
    if (userScore >= requiredScore * 0.85) return 70; // Somewhat close
    if (userScore >= requiredScore * 0.8) return 60; // Getting there
    if (userScore >= requiredScore * 0.7) return 40; // Below requirements
    return 20; // Far below requirements
};
