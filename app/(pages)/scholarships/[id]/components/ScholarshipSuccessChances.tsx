...
export const ScholarshipSuccessChances = ({ user, scholarship }: ScholarshipSuccessChancesProps) => {
  ...
  const englishSuccess = useMemo(() => {
    ...
    if (userTest === "IELTS" && scholarship.required_ielts_score) {
      const match = scholarship.required_ielts_score.match(/(?:Overall|Band)(?:\s*:)?\s*([\d.]+)/i);
      requiredScore = match ? parseFloat(match[1]) : null;
    } else if (userTest === "PTE" && scholarship.required_pte_score) {
      const match = scholarship.required_pte_score.match(/(?:Overall|Band)(?:\s*:)?\s*([\d.]+)/i);
      requiredScore = match ? parseFloat(match[1]) : null;
    } else if (userTest === "TOEFL" && scholarship.required_toefl_score) {
      const match = scholarship.required_toefl_score.match(/(?:Overall|Band)(?:\s*:)?\s*([\d.]+)/i);
      requiredScore = match ? parseFloat(match[1]) : null;
    }
  ...
  const gradeSuccess = useMemo(() => {
    ...
    const studentScale = user?.majorSubject?.previousGradingScale || "percentage";
  ...
  const nationalitySuccess = useMemo(() => {
    ...
    if (preferredNationalities.includes(userNationality.toLowerCase())) return 100; // Preferred nationality
  ...
  const academicFactors = [
    { label: "Academic Background", value: degreeSuccess, icon: "/degree-icon.svg" },
    { label: "Grades/CGPA", value: gradeSuccess, icon: "/grade-icon.svg" },
    { label: "Work Experience", value: workExperienceSuccess, icon: "/work-icon.svg" },
    { label: "English Proficiency", value: englishSuccess, icon: "/lang-icon.svg" },
  ];

  const financialFactors = [
    { label: "Nationality", value: nationalitySuccess, icon: "/nationality.svg" },
