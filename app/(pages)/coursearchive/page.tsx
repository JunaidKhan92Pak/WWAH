...
const CourseArchive = () => {
  ...
  useEffect(() => {
    ...
    const studyLevelFromURL = searchParams.get("studyLevel")
    ...
    console.log("[v0] Setting urlParamsProcessed to true")
    setUrlParamsProcessed(true)
  }, [searchParams, batchUpdateFilters])
      ...
      if (user?.favouriteCourse && Array.isArray(user.favouriteCourse)) {
        ...
        user.favouriteCourse.forEach((course: unknown) => {
          ...
          if (typeof course === "object" && course !== null) {
            courseId = (course as { _id?: string; id?: string })._id || (course as { id?: string }).id
  ...
  const toggleFavorite = async (courseId: string, action: "add" | "remove") => {
    try {
      const token = getAuthToken()
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API}favorites`, {
        method: "POST",
        headers: {
  ...
  const addToAppliedCourses = async (courseId: unknown) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API}appliedcourses`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const toggleFavoriteInDB = async (id: string) => {
    ...
    try {
      ...
      toast.success(action === "add" ? "Course added to favorites!" : "Course removed from favorites!", {
        duration: 2000,
        position: "top-center",
      })
    } catch (error) {
      ...
      setFavorites((prev) => {
        ...
        const newCount = Object.values(revertedFavorites).filter(Boolean).length
        setFavoritesCount(newCount)
  ...
  const displayedCourses = showFavorites ? Object.values(favoriteCourses) : courses

  return (
    <section className="w-[95%] mx-auto p-2 ">
      <div className="flex flex-col lg:flex-row items-start">
        <div className="w-full">
          <h3 className="font-bold mb-1 flex items-center md:mb-0">Explore Courses from Every Discipline!</h3>
        ...
        <div className="w-full md:w-[60%] lg:w-[90%] xl:w-[80%] grid grid-cols-2 md:grid-cols-4 md:flex gap-2 items-center">
          <div className="flex items-center bg-[#F1F1F1] rounded-lg px-4 w-[100%] xl:w-[90%]">
            <Image src="/search.svg" width={20} height={20} alt="search" className="w-4 h-4" />
            <Input
          ...
          >
            <Image src={favoritesCount > 0 ? "/redheart.svg" : "/hearti.svg"} width={20} height={18} alt="favorites" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:p-0">
          {displayedCourses.length === 0 ? (
            ...
            displayedCourses.map((item, idx) => (
              <div key={idx} className="bg-white shadow-xl rounded-2xl overflow-hidden flex flex-col p-3 ">
                <div className="relative h-52 p-2">
                  <Link target="blank" href={`/courses/${item._id}`} rel="noopener noreferrer" className="w-1/2">
                    <ImageWithLoader
                      src={item.universityData?.universityImages.banner || `/course-${idx || "/placeholder.svg"}.png`}
                      alt="coursesImg"
                  ...
                  <div className="absolute top-4 left-0">
                    <div className="bg-gradient-to-r from-white to-transparent opacity-100 w-[70%]">
                      <div className="flex items-center gap-2">
                        <img
                          src={item.universityData?.universityImages.logo || "/logo.png" || "/placeholder.svg"}
                          alt="alumni"
                  ...
                  <div className="absolute z-10 top-4 right-4 flex space-x-1 py-2 px-3 bg-gray-200 bg-opacity-40 backdrop-blur-sm rounded-md">
                    ...
                    <button
                      ...
                      className={`relative ${heartAnimation === item._id ? "animate-pop" : ""} ${loadingFavorites[item._id] ? "opacity-50 cursor-not-allowed" : ""
                        }`}
                    >
                      {favorites[item._id] ? (
                ...
                <div className="p-4 flex-grow">
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-y-4 md:grid-cols-2 md:gap-4">
                    ...
                    <div className="relative group flex items-center gap-2">
                      <Image src="/shop.svg" alt="intake" width={16} height={16} className="w-4 h-4" />
                    ...
                    <div className="flex items-center gap-2">
                      <Image src="/clock.svg" alt="duration" width={16} height={16} className="w-4 h-4" />
                      <p className="text-sm text-gray-600 truncate">{item.duration}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Image src="/money.svg" alt="fees" width={16} height={16} className="w-4 h-4" />
                ...
                <div className="flex justify-between items-center mb-4 mt-auto gap-2">
                  <Link target="blank" href={`/courses/${item._id}`} rel="noopener noreferrer" className="w-1/2">
                    <button className="w-full bg-red-500 text-white text-sm p-2 rounded-lg">Course Details</button>
                  </Link>
                  <div className="w-1/2">
                    <button
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex space-x-2 ">
            {(() => {
              ...
              if (startPage > 1) {
                pages.push(
                  <button key="start-ellipsis" className="rounded-lg px-4 py-2 text-gray-500" disabled>
                    ...
                  </button>,
                )
              }
              ...
              if (endPage < totalPages) {
                pages.push(
                  <button key="end-ellipsis" className="rounded-lg px-4 py-2 text-gray-500" disabled>
                    ...
                  </button>,
                )
              }
