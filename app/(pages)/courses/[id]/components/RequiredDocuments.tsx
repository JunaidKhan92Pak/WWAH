                    ...
                    <div className="bg-white p-6 rounded-xl shadow-md h-full">
                        ...
                        <ul className="grid grid-cols-2 gap-2 md:gap-4 text-gray-700">
                            {universityDocs.map((doc) => (
                                <li key={doc.name} className="flex items-center space-x-2 cursor-pointer">
                        ...
                        {selectedDocUni ? (
                            <div className="p-4">
                                ...
                                <p
                                    ...
                                    dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedDocUni.detail) }}
                                ></p>
                            </div>
                        ) : (
                            <Image
            ...
            {activeTabUni === "Embassy Documents" && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch w-[90%]">
                    ...
                    <div className="items-center justify-center rounded-3xl shadow-lg h-full block p-6 bg-red-50">
                        {selectedDoc ? (
                            <div className="p-4">
                                <h3 className="text-lg font-semibold">{selectedDoc.name} Details</h3>
                                <p
                                    className="text-gray-700 mt-2"
                                    dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedDoc.detail) }}
                                ></p>
                            </div>
                        ) : (
                            <Image
