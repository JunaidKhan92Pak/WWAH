...
const Page = () => {
  ...
  const filterUniversities = useMemo(() => {
    return universities.filter((item) => {
      ...
      const matchesSearch = item.university_name.toLowerCase().includes(normalizedSearch);
      if (selectedValues.length > 0) {
        return matchesSearch && selectedValues.includes(item.country_name);
      }
      return matchesSearch;
    });
  }, [universities, selectedValues, search]);
                ...
                <ul className="py-2 space-y-4">
                  {["USA", "China", "Canada", "Italy", "United Kingdom", "Ireland", "New Zealand", "Denmark", "France"].map((country) => (
                    <li key={country} className="flex justify-between">
                      ...
                      <input type="checkbox" id={country} value={country} onChange={handleCheckboxChange} />
                    </li>
                  ))}
                </ul>
              </ScrollArea>
