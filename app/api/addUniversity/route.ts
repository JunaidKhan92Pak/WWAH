...
export async function POST(req: Request) {
  try {
    ...
    //   return NextResponse.json({ message: "Invalid input. University details are required." }, { status: 400 });
    // }

    const uni = data // Extract university data

    // Prepare university data for insertion
    ...
    if (existingUniversity) {
      ...
      await University.findOneAndUpdate(
        { country_name: uni.country_name, university_name: uni.university_name },
        { $set: data },
        { new: true }
      );
    } else {
    ...
    return NextResponse.json({ message: "University data added/updated successfully." }, { status: 200 });
  } catch (error) {
