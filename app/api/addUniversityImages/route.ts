  ...
  try {
    ...
    if (!images || !universityName) {
      return NextResponse.json(
        { error: "Images and university name are required" },
        { status: 400 }
      );
    }
    ...
    await Promise.all(
      Object.entries(images).map(async ([imageType, base64Data]) => {
        ...
        if (typeof base64Data !== "string") {
          throw new Error(
            `Invalid data type for ${imageType}. Expected a string.`
          );
        }
        ...
        if (mimeType === "image/jpeg" || mimeType === "image/jpg") {
          ...
          throw new Error(
            `Unsupported file type: ${mimeType} for ${imageType}. Only JPEG and PNG formats are supported.`
          );
        }
