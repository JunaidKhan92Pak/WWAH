      ...
      try {
        ...
        try {
          ...
          if (dbError.code === 11000) {
            console.warn(`Duplicate course detected: ${course.course_title}`, dbError.keyValue);
            result.skipped.push({
              title: course.course_title,
...
function createUniqueFilter(course: any, country: string, university: string, universityId: string) {
  const baseFilter = {
    countryname: country,
  ...
  if (course.course_id) {
    ...
    console.log(`Using coursetiel for uniqueness: ${course.course_id}`);
    return { ...baseFilter, course_title: course.course_title };
  }
}

// Helper function to prepare course data
function prepareCoursesData(course: any, country: string, university: string, universityId: string) {
