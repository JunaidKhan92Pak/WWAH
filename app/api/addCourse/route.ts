...
type UniqueFilter =
  | UniqueFilterByCourseId
  | UniqueFilterByCourseLink
  | UniqueFilterByCourseTitle;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Get MongoDB client for hook operations
    client = new MongoClient(process.env.MONGODB_URI!);
    await client.connect();

    // Initialize database hooks
    const dbHooks = new DatabaseHooks(client);
    ...
    for (const course of data.courses) {
      try {
        ...
        const courseIdentifier = createUniqueFilter(
          course,
          normalizedCountry,
        ...
        const { updateFields, insertOnlyFields } = prepareCoursesData(
          course,
          normalizedCountry,
          normalizedUniversity,
          university._id
        ...
        // Combine all fields for the complete document
        ...
        try {
          ...
          if (existingCourse) {
            // Update existing course using database hooks
            const updateResult = await dbHooks.updateDocument(
              "courses",
              courseIdentifier as unknown as Record<string, unknown>,
              {
                $set: {
                  ...updateFields,
                  updated_at: new Date(),
            ...
            if (updateResult.modifiedCount > 0) {
              ...
              result.updated.push({
                _id: existingCourse._id.toString(),
                title: course.course_title,
            ...
            // Create new course using database hooks
            const insertResult = await dbHooks.createDocument(
              "courses",
              completeDocument
            );
          ...
          if (dbError && typeof dbError === "object" && "code" in dbError) {
            ...
            if (errorObj.code === 11000) {
              console.warn(
                `Duplicate course detected: ${course.course_title}`,
                errorObj.keyValue
              );
              result.skipped.push({
                title: course.course_title,
                reason: "Duplicate course",
                details: Array.isArray(errorObj.keyValue)
                  ? errorObj.keyValue.join(", ")
                  : String(errorObj.keyValue),
              });
            } else {
        ...
        console.error(
          `Error processing course: ${course.course_title || "Unknown"}`,
          courseError
        );
        result.failed.push({
    ...
    return NextResponse.json(
      {
        ...
        details: result,
        webhookInfo: "Embeddings will be automatically updated via webhooks",
      },
      {
...
function prepareCoursesData(
  ...
  const updateFields = {
    ...
    entry_requirements:
      course.entry_requirements || course.entry_requirement || null,
