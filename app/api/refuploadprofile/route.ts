...
export async function POST(req: Request) {
  try {
    ...
    const fileName = `ref-uploads/${Date.now()}-${file.name.replace(
      /[^a-zA-Z0-9.-]/g,
      "_"
    )}`;
    const bucketName = getRequiredEnvVar("AWS_S3_BUCKET_NAME");

    console.log("Uploading to S3 (RefUpload):", {
      bucket: bucketName,
      fileName,
      fileSize: file.size,
      fileType: file.type,
    ...
    console.log("RefUpload successful:", url);
    return NextResponse.json({ url, success: true });
  } catch (error: unknown) {
    console.error("S3 RefUpload Error:", error);
