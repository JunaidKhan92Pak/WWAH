...
const ApplicationInfo = () => {
  ...
  useEffect(() => {
    if (isDataLoaded) {
      sessionStorage.setItem("application-info-current-page", currentPage.toString())
    }
  }, [currentPage, isDataLoaded])

  // Restore current page from sessionStorage
  useEffect(() => {
    const savedPage = sessionStorage.getItem("application-info-current-page")
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (result.success && result.data) {
            ...
            const localData = sessionStorage.getItem("application-info-form")
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        clearSavedData()
        sessionStorage.removeItem("application-info-current-page")
