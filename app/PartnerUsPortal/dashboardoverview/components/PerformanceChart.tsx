...
const studentOptions = [
  "Total Students",
  "Applied Students",
  "Offer letter received students",
  "Visa granted students",
];
const exportOptions = [
  "Export CSV Data",
  "Download PNG image",
...
const PerformanceChart = () => {
  ...
  const toggleStudent = (option: string) => {
    setSelectedStudent((prev) => (prev === option ? null : option));
  };
  ...
  return (
          ...
          <div className="flex flex-col gap-2 items-start w-full pl-4 pt-2">
            ...
            <div className="flex items-center gap-2 w-full mb-4">
              ...
              <Dialog open={open} onOpenChange={setOpen}>
                ...
                >
                  <DialogHeader>
                    <DialogTitle className="mb-2 text-start">Filter Students by</DialogTitle>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button
                            ...
                            className="w-full justify-between"
                          >
                            <span>{selectedStudent ?? "Select"}</span>
                            <ChevronDown className="ml-2 h-4 w-4 text-muted-foreground" />
                          </Button>
                        </PopoverTrigger>
                        ...
                        <PopoverContent>
                          {studentOptions.map((option) => (
                            <div
                              ...
                              onClick={() => toggleStudent(option)}
                            >
                              <span>{option}</span>
                              <Checkbox
                                checked={selectedStudent === option}
                                onCheckedChange={() => toggleStudent(option)}
                              />
                            </div>
                          ))}
                        </PopoverContent>
          ...
          <p className="text-center font-semibold">
            Number of Students Over Months
          </p>

          {/* Small screens: show all ticks and smaller height */}
          <div className="block md:hidden h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={data}
                margin={{ left: 5, top: 20, right: 14, bottom: 20 }}
>
